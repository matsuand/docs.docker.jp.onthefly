...
@y
- [ログドライバー](configure.md) を利用するにあたって、そのドライバーのログ出力先が、ファイル、外部ホスト、データベース、別のログバックエンドであって、["デュアルログ"](dual-logging.md) が無効になっている場合、`docker logs`の出力は不十分なものになる場合があります。
@z
...
- If your image runs a non-interactive process such as a web server or a
  ...
  and `STDERR`.
@y
- 実行するイメージが、ウェブサーバーやデータベースのような非対話的なプロセスである場合、ログは`STDOUT`や`STDERR`にではなく、ログファイルに対して出力されるかもしれません。
@z

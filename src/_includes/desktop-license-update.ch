...
@y
    ...
    >
    > [Docker Subscription Service Agreement](https://www.docker.com/legal/docker-subscription-service-agreement){: target="_blank" rel="noopener" class="_" id="dkr_docs_desktop_install_btl"}（Docker サブスクリプションサービス契約）が更新されました。
    > [ブログ](https://www.docker.com/blog/updating-product-subscriptions/){: target="_blank" rel="noopener" class="_" id="dkr_docs_desktop_install_btl"} や [FAQ](https://www.docker.com/pricing/faq){: target="_blank" rel="noopener" class="_" id="dkr_docs_desktop_install_btl"} を読んで、Docker Desktop を利用する各企業において、どのような影響が発生するのかを確認してください。
@z
...
@y
    - Docker サブスクリプションサービス条件には、Docker Desktop の利用条件の変更が記述されています。
    - 小規模ビジネス向け（従業員 250 未満、かつ収益 1,000 万ドル未満）、個人利用、教育目的、非商用のオープンソースプロジェクトに対しては、 **無償提供が継続** されます。
    - 大規模なエンタープライズ向けのプロフェッショナル利用に対しては、有償サブスクリプションが必要です。
    - この条件の適用開始日は 2021 年 8 月31 日です。
      Docker Deskop の利用に有償サブスクリプションが必要とされるものであっても、 2022 年 1 月 31 日までは、**猶予期間** が設けられています。
    - 既存の Docker 無償サブスクリプションは **Docker Personal** として名称変更されました。
      また Docker ビジネスサブスクリプションを導入しています。
      Docker Pro、Team、Business の各サブスクリプションには、Docker Desktop の商用利用が含まれます。
@z

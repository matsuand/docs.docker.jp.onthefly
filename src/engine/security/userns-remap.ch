...
> **警告**
>
> RHEL や CentOS 7.3 などのディストリビューションにおいて、`/etc/subuid`と`/etc/subgid`に対して新たなグループの追加を自動では行わないものがあります。
...
- Using the `--privileged` mode flag on `docker run` without also specifying
  `--userns=host`.
@y
- ホストの指定（`--pid=host`または`--network=host`）を行った PID 名前空間や NET 名前空間の共有。
